...
4. 需要对照阅读时，切换为 **原文 + 译文**
5. 账户、套餐、剩余额度、自动翻译、服务地址等在 **设置** 页查看

`chrome://`、扩展商店等受限页面无法注入内容脚本，因此不可翻译。
...
| `activeTab` / `scripting` / `tabs` | 向当前页注入脚本并通信，执行翻译 |
...
3. 在 Chrome 中加载 `dist/`，验证弹窗、整页翻译、选区翻译和恢复原文
